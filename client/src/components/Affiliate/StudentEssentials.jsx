...
export default function StudentEssentials({ initialCategory = 'aptitude', viewType = 'grid' }) {
  const data = AFFILIATE_DATA.student_essentials;
  const [activeTab, setActiveTab] = useState(initialCategory);

  const handleTabChange = (key) => {
    setActiveTab(key);
    try {
      track('Affiliate Student Category Switched', {
        category_key: key,
  ...
  const activeCategoryData = data.categories[activeTab] || Object.values(data.categories)[0];
